...
def create_sample_instructor(client):
    ...
    res = client.post("/instructor", json={
        "name": "王小明",
        "email": unique_email,
...
def test_list_courses_limit(client):
    instructor_id = create_sample_instructor(client)
    for i in range(3):
        client.post("/course", json={
            "title": f"課程 {i+1}",
            "description": f"說明 {i+1}",
            "instructorId": instructor_id,
            "startTime": 1300 + i * 100,
            "endTime": 1600 + i * 100,
            "is_deleted": False,
            "deleted_at": None
        })
    ...
    assert len(data["items"]) <= 2  # 檢查最多兩筆
...
def test_update_course(client):
    ...
    print(res.status_code, res.json())  # 加這行
    course_id = res.json()["id"]

    res = client.put(f"/course/{course_id}", json={
        "title": "新標題",
        "description": "新描述",
        "instructorId": instructor_id,
        "startTime": 1400,
...
def test_update_course_not_found(client):
    ...
    fake_instructor_id = str(uuid4())
    res = client.put(f"/course/{fake_course_id}", json={
        "title": "不存在",
        "description": "錯誤",
...
def test_delete_course(client):
    ...
    res = client.delete(f"/course/{course_id}")
...
def test_delete_course_not_found(client):
    fake_course_id = str(uuid4())
    res = client.delete(f"/course/{fake_course_id}")
    assert res.status_code == 404
...
def test_list_instructor_courses(client):
    ...
    res = client.get(f"/instructor/{instructor_id}/courses")
    assert res.status_code == 200
    data = res.json()
    assert len(data) == 2
    assert data[0]["title"].startswith("A") or data[0]["title"].startswith("B")
